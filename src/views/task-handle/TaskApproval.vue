    ...
    <div class="task-list">
        ...
        <div class="task-approval">
            ...
            >
                ...
                <template #topicRender="text, record">
                ...
                <template #workflowNameRender="text, record">
                    <a v-if="filter.topic" @click="turnTaskPage(record)">
                        <template v-for="(fragment, i) in ('' + text).split(new RegExp(`(?<=${filter.topic})|(?=${filter.topic})`, 'i'))">
                            <mark v-if="fragment.toLowerCase() === filter.topic.toLowerCase()" :key="i" class="highlight">{{fragment}}</mark>
                            <template v-else>{{fragment}}</template>
                ...
                <template #actions="text, record, index">
                    ...
                    <!-- <a-button type="primary" size="small" icon="star" class="task-btn">标星</a-button>
                    <a-button type="primary" size="small" icon="undo" class="task-btn">撤回</a-button> -->
                </template>
...
export default class TaskApproval extends Vue {
    ...
    columns: Record<string, any>[] = [
        ...
        { dataIndex: 'submitter', title: '创建人', width: 100 },
        // { 
        ...
        { 
            ...
            scopedSlots: {
                filterDropdown: 'filterDropdown',
                filterIcon: 'filterIcon',
                customRender: 'workflowNameRender',
            }
        },
    ...
    handleTableChange(pagination?, filters?, sorter?) {
        ...
        this.isLoading = true;
        this.$common.post('ExtenalApi/GetPendingHandleTasksDataTable', {
            currentPage: (pagination || this.pagination).current,
            pageSize: (pagination || this.pagination).pageSize,
            filterText: '',
